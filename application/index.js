...
// Get mapping "/sso"
// Redirects user to auth0
app.get('/sso', async (req, res) => {
    const state_id = uuid.v4();
    res.redirect(`https://${secrets.domain}/authorize?response_type=code&client_id=${secrets.clientId}&redirect_uri=http://localhost:3000&scope=offline_access read:users read:current_user read:user_idp_tokens&audience=${secrets.audience}&state=${secrets.state}${state_id}`);
})

// Get mapping "/api/login-by-code"
// Returns access and refresh token using authorization code
app.get('/api/login-by-code', (req, res) => {
    const queryParams = req.query
    if (queryParams.code) {
        return res.json(JSON.parse(authenticateUserByCode(queryParams.code).getBody()))
    }
...
function authenticateUserByCode(code) {
    return request('POST', `${secrets.issuer}oauth/token`,
        {
            headers: {
                'content-type': 'application/x-www-form-urlencoded'
            },
            body: `grant_type=authorization_code&client_id=${secrets.clientId}&client_secret=${secrets.clientSecret}&code=${code}&redirect_uri=http://localhost:3000`
        })
}
